...
public:
    ...
    std::span<uint8_t> push_and_assemble(const UDPHeader& header, const std::span<uint8_t> payload)
    {
        ...
        if (payload.empty()
            || payload.size_bytes() > slice_offset
            || header.total_size == 0
            || header.total_size > pre_alloc_size
            || header.slice_idx >= slice_pre_alloc_count
            || offset >= header.total_size
            || offset + payload.size_bytes() > header.total_size)
        {
            return {};
        }
...
private:
    ...
    public:
        inline std::span<uint8_t> push_and_assemble(
            ...
            const std::span<uint8_t> slice_payload)
        {
            ...
            if (current_received_byte == total_byte)
            {
                return std::span<uint8_t>{data.data(), total_byte};
            }
            return {};
        }
